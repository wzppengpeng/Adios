...
ReflectionRegister(Layer, OutputLayer) regis_layer_output_layer("output");

void OutputLayer::init(int num_neural, int pre, Type eta) {
    l = num_neural;
    d = pre;
    m_eta = eta;
...
void OutputLayer::forward(const wzp::Matrix<Type>& pre_data) {
    //save the pre_data
    m_pre_data = &pre_data;
...
void OutputLayer::backward(const wzp::Matrix<Type>& post_diff) {
    //save the post diff
    assert(post_diff.rows() == m_post_data.rows() && post_diff.cols() == m_post_data.cols());
    m_post_diff = &post_diff; //post diff is the right labels
    //compute the post diff
    tmp_diff_from_post = std::move(post_diff - m_post_data); //m * n
    //active back
    back_active_beta();
...
void OutputLayer::back_active_beta() {
    for(size_t i = 0; i < m_post_data.rows(); ++i) {
        for(size_t j = 0; j < m_post_data.cols(); ++j) {
...
Type OutputLayer::active_fun(Type input, ActiveFucntion type) {
...
Type OutputLayer::active_fun_back(Type input, ActiveFucntion type) {
    if(type == ActiveFucntion::Relu) {
        return input > 0. ? 1 : 0;
    }
    else {
